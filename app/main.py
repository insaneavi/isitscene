...
@app.get("/releases/export.xlsx")
def export_releases_xlsx():
    """Export the complete release inventory to a formatted Excel workbook."""
    db = SessionLocal()
    try:
        releases = db.scalars(
            select(Release).order_by(Release.folder_name)
        ).all()
        ...
        workbook.set_properties({
            "title": "iSiTSCENE Release Inventory",
            "subject": "Complete release inventory export",
            "author": "iSiTSCENE",
            "comments": f"Generated by iSiTSCENE v{APP_VERSION}",
        })

        worksheet = workbook.add_worksheet("Releases")
        ...
        worksheet.autofilter(0, 0, max(len(releases), 1), len(headers) - 1)
        ...
        def local_excel_datetime(value):
            ...
            if localized is None:
                return None
            # Excel datetimes are stored without timezone metadata.
            return localized.replace(tzinfo=None)

        for row_index, release in enumerate(releases, start=1):
            values = [
                release.id,
                release.folder_name,
                release.normalized_name,
                "Present" if release.is_present else "Removed",
                "Yes" if release.is_present else "No",
                "Yes" if release.ignored else "No",
                release.verification_status,
                release.matched_release,
                release.error_message,
                local_excel_datetime(release.first_seen),
                local_excel_datetime(release.last_seen),
                local_excel_datetime(release.last_checked),
                release.review_status,
                release.review_comment,
                local_excel_datetime(release.last_reviewed),
                release.candidate_release,
                release.candidate_url,
                release.candidate_score,
                release.candidate_reason,
                local_excel_datetime(release.candidate_checked_at),
                release.imdb_id,
                release.imdb_lookup_status,
                local_excel_datetime(release.imdb_checked_at),
                release.imdb_error_message,
                release.imdb_source_release,
                release.imdb_srrdb_id,
                "Yes" if release.imdb_manual_override else "No",
                local_excel_datetime(release.imdb_manual_updated_at),
                release.movie_title,
                release.movie_year,
                release.notes,
            ]

            date_columns = {9, 10, 11, 14, 19, 22, 27}
            wrap_columns = {1, 7, 8, 13, 15, 16, 18, 23, 24, 30}
            yes_no_columns = {4, 5, 26}

            for col_index, value in enumerate(values):
                if value is None:
                    worksheet.write_blank(row_index, col_index, None, text_format)
                ...
                elif col_index in yes_no_columns:
                    worksheet.write(
                        row_index,
                        col_index,
                        value,
                        yes_format if value == "Yes" else no_format,
                    )
                elif col_index in wrap_columns:
                    worksheet.write(row_index, col_index, value, wrap_format)
                else:
                    worksheet.write(row_index, col_index, value, text_format)

        widths = [
            10, 52, 42, 15, 10, 10, 18, 52, 38, 20, 20, 20,
            16, 34, 20, 52, 45, 14, 36, 20, 16, 20, 20, 38,
            52, 16, 18, 20, 34, 12, 36,
        ]
        ...
        # Summary sheet.
        total = len(releases)
        present_count = sum(bool(r.is_present) for r in releases)
        removed_count = total - present_count
        verified_count = sum(r.verification_status == "verified" for r in releases)
        unverified_count = sum(r.verification_status == "unverified" for r in releases)
        pending_count = sum(r.verification_status == "pending" for r in releases)
        manual_imdb_count = sum(bool(r.imdb_manual_override) for r in releases)

        summary.merge_range("A1:B1", "iSiTSCENE Release Inventory Export", summary_header)
        summary.write("A3", "Metric", summary_header)
        summary.write("B3", "Count", summary_header)
        summary_rows = [
            ...
            ("Manual IMDb Overrides", manual_imdb_count),
        ]
        for idx, (label, value) in enumerate(summary_rows, start=3):
            summary.write(idx, 0, label, summary_label)
            summary.write(idx, 1, value, summary_value)

        generated_at = local_datetime(datetime.utcnow())
        summary.write("A12", "Application Version", summary_label)
        summary.write("B12", APP_VERSION)
        summary.write("A13", "Exported At", summary_label)
        if generated_at:
        ...
        summary.write("A14", "Timezone", summary_label)
        summary.write("B14", get_settings().timezone)
